...
export async function exportDocx(project: Project, options: ExportOptions): Promise<void> {
  const children: Paragraph[] = [
    ...
    new Paragraph({ children: [new TextRun({ text: `${project.genre} · 共 ${project.chapters.length} 章`, italics: true })] }),
  ];
  if (project.description) children.push(para(`简介：${project.description}`));
  ...
  for (const c of [...project.chapters].sort((a, b) => a.order - b.order)) {
    ...
    c.content.split(/\n+/).forEach((p) => { if (p.trim()) children.push(para(p.trim())); });
  }

  if (options.includeWorldbook) {
    children.push(new Paragraph({ text: '附录 · 设定集', heading: HeadingLevel.HEADING_1 }));
    const section = (title: string, rows: string[]) => {
      if (!rows.length) return;
      children.push(new Paragraph({ text: title, heading: HeadingLevel.HEADING_2 }));
      rows.forEach((r) => children.push(para(r)));
    };
