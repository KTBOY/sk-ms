    ...
    <Modal open={open} width={720} title={initial ? `编辑人物 · ${initial.name}` : '新建人物'}
      ...
      <div className="charform">
        <div className="charform__side">
          <CharacterAvatar character={draft} size={132} editable onPick={(d) => patch({ avatar: d })} />
          {draft.avatar && (
            <Button size="sm" variant="ghost" icon={<IconX size={13} />} onClick={() => patch({ avatar: null })}>移除形象</Button>
          )}
